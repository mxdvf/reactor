...
/// struct for typed errors of method [`register_op`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn register_op(configuration: &configuration::Configuration, registration_args: models::RegistrationArgs) -> Result<(), Error<RegisterOpError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_registration_args = registration_args;

    let uri_str = format!("{}/register_op", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RegisterOpError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
