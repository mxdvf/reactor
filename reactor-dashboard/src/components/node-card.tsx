...
import { Trash2 } from "lucide-react"; // Shadcn recommended icon set
...
interface NodeCardProps {
  node: Node;
  handleDeleteNode: (url: string) => void;
}
    ...
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      <Card className="@container/card">
        ...
        <CardHeader>
          ...
          <CardAction>
            <Button variant="destructive" onClick={() => handleDeleteNode(node.url)}>
        ...
        <CardContent>
          ...
          <div className="flex-1 min-w-[45%] text-left">
            <h3 className="font-medium text-gray-700 dark:text-gray-300 mb-2">Actors</h3>
          ...
          <div className="flex-1 min-w-[45%] text-right">
            <h3 className="font-medium text-gray-700 dark:text-gray-300 mb-2">Loaded Libs</h3>
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          ...
          <div className="flex justify-between w-full font-medium">
            <div>
              Latency: {node.data && node.data.latencyMs !== null ? `${node.data.latencyMs} ms` : "—"}
            </div>
            <div>
              Updated: {node.data?.lastUpdated ? new Date(node.data.lastUpdated).toLocaleTimeString() : "—"}
            </div>
          </div>
        </CardFooter>
      </Card>
